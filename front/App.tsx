...
import { parseBusinessCardImages } from "./services/api";
...
const App: React.FC = () => {
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      ...
      if (extractedDataArray.length !== files.length) {
        throw new Error(
          "Mismatch between number of files and number of results from API.",
        );
      }
      ...
      setError(
        "Не удалось обработать одну или несколько визиток. Пожалуйста, попробуйте еще раз.",
      );
      setView(View.UPLOAD);
    } finally {
      setProcessingFiles([]);
  ...
  const handleReviewComplete = (reviewedCard: BusinessCard) => {
    ...
    const remainingToReview = cardsToReview.filter(
      (c) => c.id !== reviewedCard.id,
    );
  ...
  const handleUpdateCard = (updatedCard: BusinessCard) => {
    const updatedSavedCards = savedCards.map((c) =>
